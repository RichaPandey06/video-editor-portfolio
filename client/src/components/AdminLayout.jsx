...
const AdminLayout = () => {
  ...
  useEffect(() => {
    ...
    const fetchSubscribers = async () => {
      try {
        ...
        setSubscriberCount(response.data.length); // assumes API returns array
      } catch (error) {
    ...
    const fetchProjects = async () => {
      try {
        ...
        setProjectCount(response.data.length); // assumes API returns array
      } catch (error) {
  ...
  const NAV_ITEMS = [
    ...
    { path: "/admin/projects",    label: "Projects",    icon: FolderOpen, badge: projectCount },
      ...
      <header className="sticky top-0 z-40 bg-zinc-950/90 backdrop-blur-xl border-b border-white/[0.08]">
        <div className="flex items-center justify-between px-4 sm:px-6 lg:px-10 h-16">
          ...
          <button
            ...
            className="hidden md:flex items-center gap-2 px-4 py-2 rounded-xl bg-zinc-900/60 border border-white/[0.08] hover:border-white/20 text-zinc-400 hover:text-white text-sm font-medium transition-all duration-200"
          >
